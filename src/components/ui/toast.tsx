...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Viewport
        ref={ref}
        className={cn(
            'fixed top-[10px] right-[10px] z-[100] flex max-h-screen w-full flex-col gap-2 p-0 md:max-w-[420px]',
            className
        )}
        {...props}
    />
    ...
    'data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none',
    {
        variants: {
            variant: {
                ...
                loading: 'bg-gray-50 text-gray-900 border-gray-200 [&>svg]:text-gray-600',
                destructive:
...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Close
        ref={ref}
        className={cn(
            'absolute right-1 top-1 rounded-md p-0.5 text-foreground/50 opacity-0 transition-opacity hover:text-foreground focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-red-300 group-[.destructive]:hover:text-red-50 group-[.destructive]:focus:ring-red-400 group-[.destructive]:focus:ring-offset-red-600 [.group[data-size=sm]_&]:right-1 [.group[data-size=sm]_&]:top-1',
            className
        )}
        toast-close=""
        {...props}
    >
        <X className="size-4 [.group[data-size=sm]_&]:size-3" />
    </ToastPrimitives.Close>
));
...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Title ref={ref} className={cn('text-sm font-semibold [.group[data-size=sm]_&]:text-[11px] [.group[data-size=sm]_&]:font-medium', className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
    <ToastPrimitives.Description ref={ref} className={cn('text-sm opacity-90 [.group[data-size=sm]_&]:text-[10px]', className)} {...props} />
));
